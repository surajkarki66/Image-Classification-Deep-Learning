...
import configs
import pathlib
from configs import image_height, image_width, channels
...
def generate_datasets():
    train_dataset, train_count = get_dataset(dataset_root_dir=configs.train_dir)
    valid_dataset, valid_count = get_dataset(dataset_root_dir=configs.valid_dir)
    test_dataset, test_count = get_dataset(dataset_root_dir=configs.test_dir)


    # read the original_dataset in the form of batch
    train_dataset = train_dataset.shuffle(buffer_size=train_count).batch(batch_size=configs.BATCH_SIZE)
    valid_dataset = valid_dataset.batch(batch_size=configs.BATCH_SIZE)
    test_dataset = test_dataset.batch(batch_size=configs.BATCH_SIZE)
